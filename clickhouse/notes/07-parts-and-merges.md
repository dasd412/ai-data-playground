...
## ⭐ 파트는 "INSERT 문" 단위 (행 단위 아님) → 배치 황금률

- 파트는 **INSERT 한 번**마다 1개 생김. 행 수와 무관.
  - `INSERT ... 3행` → 파트 1개. (3행 INSERT를 2번 = 파트 2개)
  - 너무 큰 INSERT(수백만 행)는 예외적으로 몇 파트로 쪼개질 수 있음 (블록 ~100만 행 단위).
- **황금률: 행 단위로 INSERT하지 말 것.** 1건씩 1만 번 = 파트 1만 개 = `too many parts` 💥
- 항상 **배치로 모아서**(수천~수만 행씩) INSERT. 실시간 로그도 버퍼에 모았다 한 번에.
